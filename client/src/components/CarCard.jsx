  ...
  return (
    <div onClick={()=> {navigate(`/car-details/${car._id}`); scrollTo(0,0)}} className='group rounded-xl overflow-hidden shadow-lg hover:-translate-y-1 transition-all duration-500 cursor-pointer'>
      ...
      <div className='relative h-48 overflow-hidden'> 
        <img src={getDisplayCarImage(car)} onError={(event) => { event.currentTarget.src = getFallbackCarImage(car) }} alt={`${car.brand} ${car.model}`} className='w-full h-full object-cover transition-transform duration-500 group-hover:scale-105'/>
