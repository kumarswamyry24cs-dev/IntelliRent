...
const Navbar = () => {
  ...
  return (
    ...
    className='sticky top-0 z-40 mx-auto flex items-center justify-between border-b border-borderColor bg-[#f8f3ec]/90 px-6 py-4 text-gray-700 backdrop-blur md:px-16 lg:px-24 xl:px-32'>
        ...
        <div className={`max-sm:fixed max-sm:h-screen max-sm:w-full max-sm:top-16 max-sm:border-t border-borderColor right-0 flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-8 max-sm:p-4 transition-all duration-300 z-50 ${location.pathname === "/" ? "bg-light" : "bg-white"} ${open ? "max-sm:translate-x-0" : "max-sm:translate-x-full"}`}>
            ...
            <div className='hidden lg:flex items-center text-sm gap-2 border border-borderColor px-3 rounded-full max-w-56 bg-white/70'>
            ...
            <div className='flex max-sm:flex-col items-start sm:items-center gap-6'>
                ...
                <button onClick={()=> {user ? logout() : setShowLogin(true)}} className="cursor-pointer rounded-full bg-primary px-8 py-2 text-white transition-all hover:bg-primary-dull">{user ? 'Logout' : 'Login'}</button>
            </div>
        </div>
